    ...
    if ($("#TypeId option:selected").val()) {
        ...
        if ($("#CurrentLinkId").text() != "") { 
            ...
            $.ajax({
                ascync: false,
                type: "POST",
                url: "/Admin/Menus/GetContentByLinkId",
                ...
                success: function (response) {
                    if (response.statusCode == 400) {
                        ...
                        for (var i = 0; i < response.data.length; i++) {
                            if (response.data[i].Value.trim() == $("#CurrentLinkId").text().trim())
                                $("#LinkIdSelectListItem").append("<option selected='selected' value='" + response.data[i].Value + "'>" + response.data[i].Text + "</option>");
                            else
