$(document).ready(function () {
    var link = $(".tree li a[data-id=" + $("#FolderId").val() + "]");
    link.addClass("SelectTree");
    $(".tree ul li a").click(function (e) {
        ...
        $("#FolderId").val($(this).attr("data-id"));
...
function successHandler(data) {
    ...
    if (data.statusCode == 200) {
        alert(data.status);
        if (data.EditOrAdd == true) {
            $("#AttachementContainer input").val(data.ID);
            $("#AttachementContainer img").attr("src", "/Content/UploadFiles/" + data.src);
            $("#AttachementContainer a").attr("href", data.src);
        }
        else {

            $("#AttachementContainer2 input").val(data.ID);
            $("#AttachementContainer2 img").attr("src", "/Content/UploadFiles/" + data.src);
            $("#AttachementContainer2 a").attr("href", data.src);
            $("#AddSecond").hide();
        }
    }
    else {
...
/********************************Uploader Single Core*********************************/
function createMode(AttachementId, EditOrAdd) {
    $('.CreateLink').hide();
    Init_Upload(AttachementId, EditOrAdd);
}
function Init_Upload(AttachementId, EditOrAdd) {
    ...
    $('#FormUpload button[id=Submit_btn]').click(function () {
        UploadFile($(this).attr("data-url") + "FileForEdit", AttachementId, EditOrAdd);
    });
...
function UploadFile(ajaxPostUrl, AttachementId, EditOrAdd) {
    ...
    dataString.append("compressionLevel", $("#compressionLevel").val());
        dataString.append("AttachementId", AttachementId);
        dataString.append("EditOrAdd", EditOrAdd);
