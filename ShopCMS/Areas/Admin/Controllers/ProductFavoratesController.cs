...
namespace ahmadi.Areas.Admin.Controllers
{
    ...
    public class ProductFavoratesController : Controller
    {
        private UnitOfWorkClass uow;
        public ProductFavoratesController()
        {
        ...
        // GET: Admin/Products
        public ActionResult Index(int? PageSize, string sortOrder, string ProductCatId, string ProductCatIdFilter, int? page)
        {
            ...
            try
            {
                ...
                if (p.Where(x => x == true).Any())
                {
                    ...
                    var Products = uow.ProductRepository.ProductItemListWithCategories(x => x.ProductFavorates.Any());
                    ...
                    switch (sortOrder)
                    {
                        ...
                        default:  // Name ascending 
                            Products = Products.OrderByDescending(s => s.Favorates);
                            break;
                    }
                    ...
                    #region EventLogger
                    ahmadi.Infrastructure.EventLog.Logger.Add(1, "ProductFavorates", "Index", true, 200, " نمایش صفحه مدیریت محصولات مورد علاقه", DateTime.Now, User.Identity.GetUserId());
                    #endregion
        ...
        public ActionResult Details(int? id, int? page)
        {
            ...
            {
                int pageNumber = (page ?? 1);
                var pf = uow.ProductFavorateRepository.Get(x => x, x => x.ProductId == id.Value,null, "Product").First();
                ViewBag.Title = pf.Product.Title;
                ViewBag.Id = pf.ProductId;
                return View(uow.ProductFavorateRepository.Get(x => x.User, x => x.ProductId == id.Value,null, "User.Avatarattachment").ToPagedList(pageNumber, 8));
            }
            else
                return RedirectToAction("Index", "AccessDenied", new System.Web.Routing.RouteValueDictionary(new { MouleName = "مدیریت محصولات" }));
