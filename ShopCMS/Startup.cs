using Microsoft.Owin;
using Owin;

[assembly: OwinStartupAttribute(typeof(ahmadi.Startup))]
namespace ahmadi
{
    public partial class Startup
    {
        public void Configuration(IAppBuilder app)
        {

            // Stimulsoft Licence Key
            //Stimulsoft.Base.StiFontCollection.AddFontFile(@"C:\Hosts\ahmadis\fonts\BNAZANIN.TTF");
            Stimulsoft.Base.StiFontCollection.AddFontFile(System.Web.HttpContext.Current.Server.MapPath("~/fonts/BNAZANIN.TTF"));
            Stimulsoft.Base.StiLicense.Key = "";
            ConfigureAuth(app);
        }
    }
}
